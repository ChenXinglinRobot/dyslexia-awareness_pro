...
const GooeyNav: React.FC<GooeyNavProps> = ({
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          try {
            ...
            // Particle may already be gone if the active item changes quickly.
          }
        }, t);
      }, 30);
    }
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number) => {
    handleSelect(e.currentTarget, index);
  };
  ...
  useEffect(() => {
    ...
    const activeItem = navRef.current.querySelectorAll("a")[activeIndex] as HTMLElement;
    if (activeItem) {
    ...
    const resizeObserver = new ResizeObserver(() => {
      const currentActiveItem = navRef.current?.querySelectorAll("a")[activeIndex] as HTMLElement;
